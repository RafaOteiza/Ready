...
public class AgregarPelicula extends javax.swing.JInternalFrame {
    ...
    String annoText = campoAnno.getText();
    String duraText = campoDuracion.getText();
    String genero = campoGenero.getText();
    
    // Validación para asegurar que los campos de año y duración contengan solo números
    if (!annoText.matches("\\d+") || !duraText.matches("\\d+")) {
        JOptionPane.showMessageDialog(null, "Por favor, ingrese solo números para Año y Duración", "Error", JOptionPane.ERROR_MESSAGE);
        return;  // Sale del método si la validación falla
    }

    int anno = Integer.parseInt(annoText);
    int dura = Integer.parseInt(duraText);
    
    // Validación adicional, por ejemplo, no permitir años negativos
    if (anno < 0) {
        JOptionPane.showMessageDialog(null, "El año no puede ser negativo", "Error", JOptionPane.ERROR_MESSAGE);
        return;  // Sale del método si la validación falla
    }
    
    // Validación adicional, por ejemplo, no permitir duraciones negativas
    if (dura < 0) {
        JOptionPane.showMessageDialog(null, "La duración no puede ser negativa", "Error", JOptionPane.ERROR_MESSAGE);
        return;  // Sale del método si la validación falla
    }
